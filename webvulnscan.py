...
# For critical errors and crashes

    BG_ERR_TXT 	= '\033[41m' 
    BG_HEAD_TXT = '\033[46m'
    BG_ENDL_TXT = '\033[100m'
    BG_CRIT_TXT = '\033[45m'
    BG_HIGH_TXT = '\033[41m'
    BG_MED_TXT  = '\033[43m'
    BG_LOW_TXT  = '\033[44m'
...
def vul_info(val):
	result =''
	if val == 'c':
		result = b_colours.BG_CRIT_TXT+" critical "+b_colours.ENDC
	elif val == 'h':
		result = b_colours.BG_HIGH_TXT+" high "+b_colours.ENDC
	elif val == 'm':
		result = b_colours.BG_MED_TXT+" medium "+b_colours.ENDC
	elif val == 'l':
		result = b_colours.BG_LOW_TXT+" low "+b_colours.ENDC
	else:
		result = b_colours.BG_INFO_TXT+" info "+b_colours.ENDC
	return result

# Legends
legends_high = b_colours.BADFAIL + "●" + b_colours.ENDC
legends_med  = b_colours.WARNING + "●" + b_colours.ENDC
legends_low  = b_colours.okgreen + "●" + b_colours.ENDC

# Links the vulnerability with threat level and remediation database
def vul_remed_info(v1,v2,v3):
	print b_colours.BOLD+"Vulnerability Threat Level"+b_colours.ENDC
	print "\t"+vul_info(v2)+" "+b_colours.WARNING+str(tool_resp[v1][0])+b_colours.ENDC
	print b_colours.BOLD+"Vulnerability Definition"+b_colours.ENDC
	print "\t"+b_colours.BADFAIL+str(tools_fix[v3-1][1])+b_colours.ENDC
	print b_colours.BOLD+"Vulnerability Remediation"+b_colours.ENDC
	print "\t"+b_colours.okgreen+str(tools_fix[v3-1][2])+b_colours.ENDC


# webvulnscan Help Context
def helper():
        print b_colours.okblue+"Information:"+b_colours.ENDC
        print "------------"
        print "\t./webvulnscan.py example.com: Scans the domain example.com"
        print "\t./webvulnscan.py --update   : Updates the scanner to the latest version."
        print "\t./webvulnscan.py --help     : Displays this help context."
        print b_colours.okblue+"Interactive:"+b_colours.ENDC
        print "------------"
        print "\tCtrl+C: Skips current test."
        print "\tCtrl+Z: Quits webvulnscan."
        print b_colours.okblue+"Legends:"+b_colours.ENDC
        print "--------"
        print "\t["+legends_high+"]: Scan process may take longer times (not predictable)."
        print "\t["+legends_med+"]: Scan process may take less than 10 minutes."
        print "\t["+legends_low+"]: Scan process may take less than a minute or two."
        print b_colours.okblue+"Vulnerability Information:"+b_colours.ENDC
        print "--------------------------"
...
# webvulnscan Logo
def logo():
	print b_colours.WARNING
        print(b_colours.BADFAIL+"●"+b_colours.WARNING+"""WEBVULNSCAN"""+"●"+b_colours.ENDC+"""\n(The Web Vulnerability Scanner""")
        print b_colours.ENDC
    ...
    def spinner_task(self):
        try:
            while self.busy:
                ...
                print b_colours.BG_ERR_TXT+next(self.spinner_generator)+b_colours.ENDC,
                sys.stdout.flush()
            ...
            #clear()
            print "\n\t"+ b_colours.BG_ERR_TXT+"webvulnscan received a series of Ctrl+C hits. Quitting..." +b_colours.ENDC
            sys.exit(1)
...
tool_status = [
                ["has IPv6",1,legends_low," < 15s","ipv6",["not found","has IPv6"]],
                ["Server Error",0,legends_low," < 30s","asp.netmisconf",["unable to resolve host address","Connection timed out"]],
                ["wp-login",0,legends_low," < 30s","wpcheck",["unable to resolve host address","Connection timed out"]],
                ["drupal",0,legends_low," < 30s","drupalcheck",["unable to resolve host address","Connection timed out"]],
                ["joomla",0,legends_low," < 30s","joomlacheck",["unable to resolve host address","Connection timed out"]],
                ["[+]",0,legends_low," < 40s","robotscheck",["Use of uninitialized value in unpack at"]],
                ["No WAF",0,legends_low," < 45s","wafcheck",["appears to be down"]],
                ["tcp open",0,legends_med," <  2m","nmapopen",["Failed to resolve"]],
                ["No emails found",1,legends_med," <  3m","harvester",["No hosts found","No emails found"]],
                ["[+] Zone Transfer was successful!!",0,legends_low," < 20s","dnsreconzt",["Could not resolve domain"]],
                ["Whoah, it worked",0,legends_low," < 30s","fiercezt",["none"]],
                ["0 errors",0,legends_low," < 35s","dnswalkzt",["!!!0 failures, 0 warnings, 3 errors."]],
                ["Admin Email:",0,legends_low," < 25s","whois",["No match for domain"]],
                ["XSS filter is disabled",0,legends_low," < 20s","nmapxssh",["Failed to resolve"]],
                ["VULNERABLE",0,legends_high," < 45m","nmapdos",["Failed to resolve"]],
                ["Server is vulnerable to Heartbleed",0,legends_low," < 40s","sslyzehb",["Could not resolve hostname"]],
                ["VULNERABLE",0,legends_low," < 30s","nmap1",["Failed to resolve"]],
                ["VULNERABLE",0,legends_low," < 35s","nmap2",["Failed to resolve"]],
                ["VULNERABLE",0,legends_low," < 35s","nmap3",["Failed to resolve"]],
                ["VULNERABLE",0,legends_low," < 30s","nmap4",["Failed to resolve"]],
                ["VULNERABLE",0,legends_low," < 35s","nmap5",["Failed to resolve"]],
                ["ERROR - OCSP response status is not successful",0,legends_low," < 25s","sslyze1",["Could not resolve hostname"]],
                ["VULNERABLE",0,legends_low," < 30s","sslyze2",["Could not resolve hostname"]],
                ["VULNERABLE",0,legends_low," < 25s","sslyze3",["Could not resolve hostname"]],
                ["VULNERABLE",0,legends_low," < 30s","sslyze4",["Could not resolve hostname"]],
                ["does NOT use Load-balancing",0,legends_med," <  4m","lbd",["NOT FOUND"]],
                ["No vulnerabilities found",1,legends_low," < 45s","golism1",["Cannot resolve domain name","No vulnerabilities found"]],
                ["No vulnerabilities found",1,legends_low," < 40s","golism2",["Cannot resolve domain name","No vulnerabilities found"]],
                ["No vulnerabilities found",1,legends_low," < 45s","golism3",["Cannot resolve domain name","No vulnerabilities found"]],
                ["No vulnerabilities found",1,legends_low," < 40s","golism4",["Cannot resolve domain name","No vulnerabilities found"]],
                ["No vulnerabilities found",1,legends_low," < 45s","golism5",["Cannot resolve domain name","No vulnerabilities found"]],
                ["FOUND: 0",1,legends_high," < 35m","dirb",["COULDNT RESOLVE HOST","FOUND: 0"]],
                ["Could not find any vulnerability!",1,legends_med," <  4m","xsser",["XSSer is not working propertly!","Could not find any vulnerability!"]],
                ["Occurrence ID",0,legends_low," < 45s","golism6",["Cannot resolve domain name"]],
                ["DNS zone transfer successful",0,legends_low," < 30s","golism7",["Cannot resolve domain name"]],
                ["Nikto found 0 vulnerabilities",1,legends_med," <  4m","golism8",["Cannot resolve domain name","Nikto found 0 vulnerabilities"]],
                ["Possible subdomain leak",0,legends_high," < 30m","golism9",["Cannot resolve domain name"]],
                ["AXFR record query failed:",1,legends_low," < 45s","dnsenumzt",["NS record query failed:","AXFR record query failed","no NS record for"]],
                ["Found 0 entries",1,legends_high," < 75m","fierce2",["Found 0 entries","is gimp"]],
                ["Found 0 E-Mail(s)",1,legends_low," < 30s","dmitry1",["Unable to locate Host IP addr","Found 0 E-Mail(s)"]],
                ["Found 0 possible subdomain(s)",1,legends_low," < 35s","dmitry2",["Unable to locate Host IP addr","Found 0 possible subdomain(s)"]],
                ["open",0,legends_low," < 15s","nmaptelnet",["Failed to resolve"]],
                ["open",0,legends_low," < 15s","nmapftp",["Failed to resolve"]],
                ["open",0,legends_low," < 20s","nmapstux",["Failed to resolve"]],
                ["SUCCEED",0,legends_low," < 30s","webdav",["is not DAV enabled or not accessible."]],
                ["No vulnerabilities found",1,legends_low," < 15s","golism10",["Cannot resolve domain name","No vulnerabilities found"]],
                ["[+]",0,legends_med," <  2m","uniscan2",["Use of uninitialized value in unpack at"]],
                ["[+]",0,legends_med," <  5m","uniscan3",["Use of uninitialized value in unpack at"]],
                ["[+]",0,legends_med," <  9m","uniscan4",["Use of uninitialized value in unpack at"]],
                ["[+]",0,legends_med," <  8m","uniscan5",["Use of uninitialized value in unpack at"]],
                ["[+]",0,legends_med," <  9m","uniscan6",["Use of uninitialized value in unpack at"]],
                ["0 item(s) reported",1,legends_low," < 35s","nikto1",["ERROR: Cannot resolve hostname","0 item(s) reported","No web server found","0 host(s) tested"]],
                ["0 item(s) reported",1,legends_low," < 35s","nikto2",["ERROR: Cannot resolve hostname","0 item(s) reported","No web server found","0 host(s) tested"]],
                ["0 item(s) reported",1,legends_low," < 35s","nikto3",["ERROR: Cannot resolve hostname","0 item(s) reported","No web server found","0 host(s) tested"]],
                ["0 item(s) reported",1,legends_low," < 35s","nikto4",["ERROR: Cannot resolve hostname","0 item(s) reported","No web server found","0 host(s) tested"]],
                ["0 item(s) reported",1,legends_low," < 35s","nikto5",["ERROR: Cannot resolve hostname","0 item(s) reported","No web server found","0 host(s) tested"]],
                ["0 item(s) reported",1,legends_low," < 35s","nikto6",["ERROR: Cannot resolve hostname","0 item(s) reported","No web server found","0 host(s) tested"]],
                ["0 item(s) reported",1,legends_low," < 35s","nikto7",["ERROR: Cannot resolve hostname","0 item(s) reported","No web server found","0 host(s) tested"]],
                ["0 item(s) reported",1,legends_low," < 35s","nikto8",["ERROR: Cannot resolve hostname","0 item(s) reported","No web server found","0 host(s) tested"]],
                ["0 item(s) reported",1,legends_low," < 35s","nikto9",["ERROR: Cannot resolve hostname","0 item(s) reported","No web server found","0 host(s) tested"]],
                ["0 item(s) reported",1,legends_low," < 35s","nikto10",["ERROR: Cannot resolve hostname","0 item(s) reported","No web server found","0 host(s) tested"]],
                ["0 item(s) reported",1,legends_low," < 35s","nikto11",["ERROR: Cannot resolve hostname","0 item(s) reported","No web server found","0 host(s) tested"]],
                ["0 item(s) reported",1,legends_low," < 35s","nikto12",["ERROR: Cannot resolve hostname","0 item(s) reported","No web server found","0 host(s) tested"]],
                ["0 item(s) reported",1,legends_low," < 35s","nikto13",["ERROR: Cannot resolve hostname","0 item(s) reported","No web server found","0 host(s) tested"]],
                ["0 item(s) reported",1,legends_low," < 35s","nikto14","ERROR: Cannot resolve hostname , 0 item(s) reported"],
                ["#1",0,legends_high," < 30m","dnsmap_brute",["[+] 0 (sub)domains and 0 IP address(es) found"]],
                ["open",0,legends_low," < 15s","nmapmssql",["Failed to resolve"]],
                ["open",0,legends_low," < 15s","nmapmysql",["Failed to resolve"]],
                ["open",0,legends_low," < 15s","nmaporacle",["Failed to resolve"]],
                ["open",0,legends_low," < 15s","nmapudprdp",["Failed to resolve"]],
                ["open",0,legends_low," < 15s","nmaptcprdp",["Failed to resolve"]],
                ["open",0,legends_high," > 50m","nmapfulltcp",["Failed to resolve"]],
                ["open",0,legends_high," > 75m","nmapfulludp",["Failed to resolve"]],
                ["open",0,legends_low," < 30s","nmapsnmp",["Failed to resolve"]],
                ["Microsoft SQL Server Error Log",0,legends_low," < 30s","elmahxd",["unable to resolve host address","Connection timed out"]],
                ["open",0,legends_low," < 20s","nmaptcpsmb",["Failed to resolve"]],
                ["open",0,legends_low," < 20s","nmapudpsmb",["Failed to resolve"]],
                ["Host:",0,legends_med," < 5m","wapiti",["none"]],
                ["WebDAV is ENABLED",0,legends_low," < 40s","nmapwebdaviis",["Failed to resolve"]],
                ["X-XSS-Protection[1",1,legends_med," < 3m","whatweb",["Timed out","Socket error","X-XSS-Protection[1"]]
...
tools_fix = [
					...
					[30, "Particular Scanner found multiple vulnerabilities that an attacker may try to exploit the target.",
							"Refer to WS-Vulnerability-Report to view the complete information of the vulnerability, once the scan gets completed."],
...
# Tool Head Pointer: (can be increased but some tools will be skipped)
tool = 0
...
if len(sys.argv) == 1 :
    ...
    if target == '--update' or target == '-u' or target == '--u':
    	logo()
        print "webvulnscan is updating....Please wait.\n"
        spinner.start()
        # Checking internet connectivity first...
        rs_internet_availability = check_internet()
        if rs_internet_availability == 0:
            print "\t"+ b_colours.BG_ERR_TXT + "There seems to be some problem connecting to the internet. Please try again or later." +b_colours.ENDC
            spinner.stop()
            sys.exit(1)
        cmd = 'sha1sum webvulnscan.py | grep .... | cut -c 1-40'
        oldversion_hash = subprocess.check_output(cmd, shell=True)
        oldversion_hash = oldversion_hash.strip()
        os.system('wget -N https://raw.githubusercontent.com/rishabhrsjain/webvulnscan/master/webvulnscan.py -O webvulnscan.py > /dev/null 2>&1')
        ...
        if oldversion_hash == newversion_hash :
            clear()
            print "\t"+ b_colours.okblue +"You already have the latest version of webvulnscan." + b_colours.ENDC
        else:
            clear()
            print "\t"+ b_colours.okgreen +"webvulnscan successfully updated to the latest version." +b_colours.ENDC
        spinner.stop()
        sys.exit(1)
        ...
        logo()
        print b_colours.BG_HEAD_TXT+"[ Checking Available Security Scanning Tools Phase..... Initiated. ]"+b_colours.ENDC
        unavail_tools = 0
        unavail_tools_names = list()
        while (ws_avail_tools < len(tools_precheck)):
			precmd = str(tools_precheck[ws_avail_tools][arg1])
			try:
				p = subprocess.Popen([precmd], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE,shell=True)
				output, err = p.communicate()
				val = output + err
			except:
				print "\t"+b_colours.BG_ERR_TXT+"webvulnscan was terminated abruptly....."+b_colours.ENDC
				sys.exit(1)
			if "not found" in val:
				print "\t"+b_colours.okblue+tools_precheck[ws_avail_tools][arg1]+b_colours.ENDC+b_colours.BADFAIL+".....unavailable."+b_colours.ENDC
				for scanner_index, scanner_val in enumerate(tool_names):
					if scanner_val[2] == tools_precheck[ws_avail_tools][arg1]:
						scanner_val[3] = 0 # disabling scanner as it's not available.
						unavail_tools_names.append(tools_precheck[ws_avail_tools][arg1])
						unavail_tools = unavail_tools + 1
			else:
				print "\t"+b_colours.okblue+tools_precheck[ws_avail_tools][arg1]+b_colours.ENDC+b_colours.okgreen+".....available."+b_colours.ENDC
			ws_avail_tools = ws_avail_tools + 1
			clear()
        unavail_tools_names = list(set(unavail_tools_names))
        if unavail_tools == 0:
        	print "\t"+b_colours.okgreen+"All Scanning Tools are available. All vulnerability checks will be performed by webvulnscan."+b_colours.ENDC
        else:
        	print "\t"+b_colours.WARNING+"Some of these tools "+b_colours.BADFAIL+str(unavail_tools_names)+b_colours.ENDC+b_colours.WARNING+" are unavailable. webvulnscan can still perform tests by excluding these tools from the tests. Please install these tools to fully utilize the functionality of webvulnscan."+b_colours.ENDC
        print b_colours.BG_ENDL_TXT+"[ Checking Available Security Scanning Tools Phase... Completed. ]"+b_colours.ENDC
        print "\n"
        print b_colours.BG_HEAD_TXT+"[ Preliminary Scan Phase Initiated..... Loaded "+str(tool_checks)+" vulnerability checks.  ]"+b_colours.ENDC
        #while (tool < 1):
        while(tool < len(tool_names)):
            print "["+tool_status[tool][arg3]+tool_status[tool][arg4]+"] Deploying "+str(tool+1)+"/"+str(tool_checks)+" | "+b_colours.okblue+tool_names[tool][arg2]+b_colours.ENDC,
            if tool_names[tool][arg4] == 0:
            	print b_colours.WARNING+".....Scanning Tool Unavailable. Auto-Skipping Test....."+b_colours.ENDC
		ws_skipped_checks = ws_skipped_checks + 1
            	tool = tool + 1
            	continue
            spinner.start()
            ...
            if runTest == 1:
                    ...
                    ws_total_elapsed = ws_total_elapsed + elapsed
                    print b_colours.okblue+"\b.....Completed in "+display_time(int(elapsed))+b_colours.ENDC+"\n"
                    clear()
                    ...
                    if tool_status[tool][arg2] == 0:
                    	if tool_status[tool][arg1].lower() in rs_tool_output_file.lower():
                        	#print "\t"+ vul_info(tool_resp[tool][arg2]) + b_colours.BADFAIL +" "+ tool_resp[tool][arg1] + b_colours.ENDC
                        	vul_remed_info(tool,tool_resp[tool][arg2],tool_resp[tool][arg3])
                        	ws_vul_list.append(tool_names[tool][arg1]+"*"+tool_names[tool][arg2])
                    else:
                    ...
                    elapsed = scan_stop - scan_start
                    ws_total_elapsed = ws_total_elapsed + elapsed
                    print b_colours.okblue+"\b\b\b\b...Interrupted in "+display_time(int(elapsed))+b_colours.ENDC+"\n"
                    clear()
                    print "\t"+b_colours.WARNING + "Test Skipped. Performing Next. Press Ctrl+Z to Quit webvulnscan." + b_colours.ENDC
                    ws_skipped_checks = ws_skipped_checks + 1

            tool=tool+1

        print b_colours.BG_ENDL_TXT+"[ Preliminary Scan Phase Completed. ]"+b_colours.ENDC
        print "\n"

        #################### Report & Documentation Phase ###########################
        print b_colours.BG_HEAD_TXT+"[ Report Generation Phase Initiated. ]"+b_colours.ENDC
        if len(ws_vul_list)==0:
        	print "\t"+b_colours.okgreen+"No Vulnerabilities Detected."+b_colours.ENDC
        else:
        	with open("WS-Vulnerability-Report", "a") as report:
        		while(ws_vul < len(ws_vul_list)):
        			vuln_info = ws_vul_list[ws_vul].split('*')
	       		...
	       		print "\tComplete Vulnerability Report for "+b_colours.okblue+target+b_colours.ENDC+" named "+b_colours.okgreen+"`WS-Vulnerability-Report`"+b_colours.ENDC+" is available under the same directory webvulnscan resides."

        	report.close()
        # Writing all scan files output into WS-Debug-ScanLog for debugging purposes.
        for file_index, file_name in enumerate(tool_names):
        	with open("WS-Debug-ScanLog", "a") as report:
        		try:
        ...
        print "\tTotal Number of Vulnerability Checks Done        : "+b_colours.BOLD+b_colours.okgreen+str(len(tool_names))+b_colours.ENDC
        print "\tTotal Number of Vulnerability Checks Skipped     : "+b_colours.BOLD+b_colours.WARNING+str(ws_skipped_checks)+b_colours.ENDC
        print "\tTotal Number of Vulnerabilities Detected         : "+b_colours.BOLD+b_colours.BADFAIL+str(len(ws_vul_list))+b_colours.ENDC
        print "\tTotal Time Elapsed for the Scan                  : "+b_colours.BOLD+b_colours.okblue+display_time(int(ws_total_elapsed))+b_colours.ENDC
        print "\n"
        print "\tFor Debugging Purposes, You can view the complete output generated by all the tools named "+b_colours.okblue+"`WS-Debug-ScanLog`"+b_colours.ENDC+" under the same directory."
        print b_colours.BG_ENDL_TXT+"[ Report Generation Phase Completed. ]"+b_colours.ENDC
